...
python3 -m json.tool "$ROOT/templates/agent-assets/macos-signals-review.example.json" >/dev/null
...
AGENT_ASSETS_USER_HOME="$TMP_HOME" "$TMP_HOME/.local/bin/agent-assets-macos-signals" >/dev/null
python3 -m json.tool "$TMP_HOME/.config/agent-assets/macos-signals.json" >/dev/null
