...
> **定位注脚（2026-06）**：这份分类稿仍有效（dashboard 展示资产时用），但**登记 / 分类已降为尽力而为的次要工作**。
> 项目核心转向**运行态观测**（见 README / architecture 的 Runtime Observation）。registry 退役为历史快照，
> 不再要求"每装一个就登记"——实测登记必然脱节（见下方"核心病根"，那就是它失败的证据）。
> 分类模型留给愿意维护台账时用，不阻塞观测。
...
> 8 分类主类型已落地为 dashboard 的资产分组（代码 `asset_group`），分组顺序：
> `agent` / `mcp` / `cli-tool` / `skill` / `project` / `config` / `support` / `unsorted`（临时区，目标是清零）。
> 形态标签、MCP 按宿主分组、总览突出"待处理"等设计点在 dashboard 里渐进实现。
> `recategorize-proposal.md` / `dry-run-recategorize.md` 是落地前对本机 58 条资产的逐条归类提议，保留作决策记录；逐条确认已被通用的 8 分类分组取代。
