...
### [FEAT] 系统进程页内存总览新增「未覆盖进程」条形

| 字段 | 内容 |
|---|---|
| **问题/需求** | 用户反馈系统进程页顶部内存总览与下方进程列表对不上，但不知道具体差在哪儿、「哪些东西穿透不进去」。 |
| **根因/方案** | 进程列表只采集用户态进程 RSS，而系统总已用内存还包含 root 守护进程、kernel/wired、压缩内存、共享库/GPU 等。在内存总览卡片中新增「未覆盖进程」条形（值 = 系统已用 − 列表 RSS 合计），并把底部说明改为具体清单。 |
| **改动范围** | `lib/agent_assets_dashboard_render.py`（`_render_memory_summary_card` 计算与渲染）、`lib/agent_assets_dashboard_html.py`（`.uncovered` 样式）、`tests/test_dashboard_render.py`、`docs/CHANGELOG.md` |
| **影响面** | 系统进程页一眼可见「上面总内存」与「下面进程列表」之间的缺口大小；未覆盖部分用灰色条形独立展示。 |
| **状态** | ✅ 已完成 |
...
| 字段 | 内容 |
|---|---|
| **问题/需求** | 系统信号 tab 顶部出现「全部状态 / 运行中 / 未运行 / 登录项/扩展」按钮行，用户觉得杂乱，希望像 Excel 那样点击表头排序/筛选。 |
| **根因/方案** | 移除单独的状态筛选按钮行；把系统信号表格的「名称 / 类型 / 状态 / 资源」表头改为可点击排序；状态列按 running / not-running / registered 排序，点一下即可把所有「运行中」或「未运行」集中在一起。 |
| **改动范围** | `lib/agent_assets_dashboard_render.py`、`lib/agent_assets_dashboard_html.py`、`docs/CHANGELOG.md` |
| **影响面** | 系统信号 tab 顶部更清爽；表格支持表头排序；控制筛选按钮（全部 / 你的·可开关 / 系统级·别动 / 在跑）保留。 |
| **状态** | ✅ 已完成 |
